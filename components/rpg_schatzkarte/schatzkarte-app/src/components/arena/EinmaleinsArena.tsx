...
import { useLearningGroups } from '@/hooks';
...
export function EinmaleinsArena({ isOpen, onClose, onXPEarned, onCoinsEarned }: ArenaProps) {
  const { isCoach, legacyUserId } = useAuth();
  const userId = legacyUserId ?? '';
  const { data: coachGroups } = useLearningGroups();
  const coachGroupIds = (coachGroups ?? []).map(g => g.group_id);
  ...
  useEffect(() => {
    if (!userId || supabaseLoaded) return;
      ...
      try {
        ...
        if (data) {
          ...
          if (localState.playerName || localState.totalCorrect > 0) {
            await saveToSupabase(userId, localState);
          }
        }
      } catch (e) {
    ...
    loadFromSupabase();
  }, [userId, supabaseLoaded]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const debouncedSaveToSupabase = useCallback((nextState: ArenaState) => {
    ...
    saveTimerRef.current = setTimeout(() => {
      saveToSupabase(userId, nextState);
    }, 2000);
  }, [userId]);
  ...
  const flushSaveToSupabase = useCallback(() => {
    if (!userId) return;
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    saveToSupabase(userId, stateRef.current);
  }, [userId]);
  ...
  return (
    <div style={modalOverlay} onClick={handleClose}>
      <div style={modalContent} onClick={e => e.stopPropagation()}>
        ...
        {screen === 'home' && <HomeScreen state={state} onNavigate={navigate} onUpdate={updateState} isCoach={isCoach} />}
        {screen === 'quiz' && (
          <QuizGame
            state={state}
        ...
        {screen === 'coach' && isCoach && (
          <CoachScreen groupIds={coachGroupIds} onBack={() => setScreen('home')} />
        )}
      </div>
...
async function saveToSupabase(userId: string, s: ArenaState) {
  try {
...
function CoachScreen({ groupIds, onBack }: {
  groupIds: string[]; // TODO: implement group IDs from auth/context
  onBack: () => void;
}) {
...
export default EinmaleinsArena;
