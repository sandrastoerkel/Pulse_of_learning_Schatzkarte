...
// Unterstuetzt: single, multi-select, matching, ordering
// ============================================
...
import {
  ...
  ExtendedQuiz
} from '@/types/legacy-ui';
import { POINTS_PER_TYPE, getWorldInfo } from '@/content/festungQuizContent';
...
const MONSTERS = [
  ...
  { name: 'Raetsel-Wolf', icon: '\uD83D\uDC3A', baseHp: 5, xpBonus: 30 },
...
export function BattleQuiz({
  ...
  const dynamicHp = Math.max(
    baseMonster.baseHp,
    Math.ceil(quiz.questions.length * 1.3)
  );
  ...
  const processAnswer = useCallback((isCorrect: boolean, _points: number) => {
    if (isCorrect) {
  ...
  // Antwort verarbeiten - Single-Choice
  ...
  }, [selectedAnswer, battleState, currentQuestion, processAnswer]);
  ...
  const checkMultiSelect = useCallback(() => {
    ...
    processAnswer(isCorrect, getPointsForQuestion());
  }, [currentQuestion, selectedMultiAnswers, processAnswer]);
  ...
  const checkMatching = useCallback(() => {
    ...
    setSelectedAnswer(isCorrect ? 1 : 0);
    processAnswer(isCorrect, getPointsForQuestion());
  }, [currentQuestion, matchingState, processAnswer]);
  ...
  const checkOrdering = useCallback(() => {
    ...
    setSelectedAnswer(isCorrect ? 1 : 0);
    processAnswer(isCorrect, getPointsForQuestion());
  }, [currentQuestion, orderingState, processAnswer]);
      ...
      <div className="battle-arena">
        <div className="battle-result game-over">
          ...
          <div className="game-over-message">
            <p>{'\uD83E\uDD14'} "Was kann ich beim naechsten Mal anders machen?"</p>
    ...
    return (
      <div className="battle-arena">
        <div className={`battle-result ${battleState.isVictory ? 'victory' : 'defeat'}`}>
          ...
          <h2>{battleState.isVictory ? 'GEWONNEN!' : 'Niederlage...'}</h2>
          ...
          <div className="battle-stats">
            ...
            {enableLives && (
              <div className="stat">
                <span className="stat-label">Leben uebrig</span>
        ...
        {questionType === 'multi-select' && (
          <div className="multi-select-container">
            ...
            {selectedAnswer === null && (
              ...
              >
                {'\u2713'} PRUEFEN ({selectedMultiAnswers.length}/{(currentQuestion as MultiSelectQuestion).correctCount})
              </button>
            )}
          </div>
        )}
