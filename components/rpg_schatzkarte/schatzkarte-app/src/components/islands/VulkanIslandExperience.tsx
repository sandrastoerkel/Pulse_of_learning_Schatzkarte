...
const QUEST_INFO: Record<QuestKey, { name: string; icon: string; color: string; description: string; xp: number }> = {
  video: { name: 'Weisheit erlangen', icon: '📜', color: '#ff7043', description: 'Schau dir das Lernvideo an!', xp: 25 },
  scroll: { name: 'Schriftrolle studieren', icon: '📖', color: '#f4511e', description: 'Lerne über Motivation!', xp: 20 },
  quiz: { name: 'Wissen testen', icon: '⚔️', color: '#e64a19', description: 'Teste dein Wissen!', xp: 50 },
  challenge: { name: 'Motivations-Feuer', icon: '🔥', color: '#bf360c', description: 'Entfache dein inneres Feuer!', xp: 40 },
};
    ...
    <div className="vulkan-island">
      <div className="island-header">
        ...
        <h1 className="island-title">
          <motion.div className="title-icon" animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 2 }}>
            <VulkanIcon size={40} animated />
          </motion.div>
          Was dich antreibt
        </h1>
