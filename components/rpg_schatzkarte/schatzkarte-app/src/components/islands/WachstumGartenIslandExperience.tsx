...
import { WachstumGartenIcon } from '@/components/icons';
...
const ISLAND_ID = 'wachstum_garten';
...
const QUEST_INFO: Record<QuestKey, { name: string; icon: string; color: string; description: string; xp: number }> = {
  video: { name: 'Weisheit erlangen', icon: '📜', color: '#c5e1a5', description: 'Schau dir das Lernvideo an!', xp: 25 },
  scroll: { name: 'Schriftrolle studieren', icon: '📖', color: '#aed581', description: 'Lerne über Wachstumsdenken!', xp: 20 },
  quiz: { name: 'Wissen testen', icon: '⚔️', color: '#9ccc65', description: 'Teste dein Wissen!', xp: 50 },
  challenge: { name: 'Wachstums-Experiment', icon: '🌱', color: '#8bc34a', description: 'Lass dein Mindset wachsen!', xp: 40 },
};

export function WachstumGartenIslandExperience() {
    ...
    <div className="wachstum-garten-island">
      <div className="island-header">
        ...
        <h1 className="island-title">
          <motion.div className="title-icon" animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 3 }}><WachstumGartenIcon size={40} animated /></motion.div>
