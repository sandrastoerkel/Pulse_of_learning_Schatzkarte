...
import { SchutzBurgIcon } from '@/components/icons';
...
const QUEST_INFO: Record<QuestKey, { name: string; icon: string; color: string; description: string; xp: number }> = {
  ...
  scroll: { name: 'Schriftrolle studieren', icon: '📖', color: '#f06292', description: 'Lerne über Mobbing-Schutz!', xp: 20 },
  quiz: { name: 'Wissen testen', icon: '⚔️', color: '#ec407a', description: 'Teste dein Wissen!', xp: 50 },
  challenge: { name: 'Grenzen-Training', icon: '🛡', color: '#e91e63', description: 'Lerne Grenzen zu setzen!', xp: 40 },
};

export function SchutzBurgIslandExperience() {
    ...
    <div className="schutz-burg-island">
      <div className="island-header">
        ...
        <h1 className="island-title">
          <motion.div className="title-icon" animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 3 }}><SchutzBurgIcon size={40} animated /></motion.div>
      ...
      <motion.div className="island-subtitle-bar" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <span className="subtitle-text">(Mobbing-Schutz &amp; Grenzen setzen)</span>
      </motion.div>
