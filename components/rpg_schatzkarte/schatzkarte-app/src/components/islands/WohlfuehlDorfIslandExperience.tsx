...
import { WohlfuehlDorfIcon } from '@/components/icons';
...
const ISLAND_ID = 'wohlfuehl_dorf';
...
const QUEST_INFO: Record<QuestKey, { name: string; icon: string; color: string; description: string; xp: number }> = {
  video: { name: 'Weisheit erlangen', icon: '📜', color: '#a5d6a7', description: 'Schau dir das Lernvideo an!', xp: 25 },
  scroll: { name: 'Schriftrolle studieren', icon: '📖', color: '#81c784', description: 'Lerne über Zugehörigkeit!', xp: 20 },
  quiz: { name: 'Wissen testen', icon: '⚔️', color: '#66bb6a', description: 'Teste dein Wissen!', xp: 50 },
  challenge: { name: 'Wohlfühl-Aktion', icon: '🏡', color: '#4caf50', description: 'Schaffe dir einen Wohlfühlort!', xp: 40 },
};

export function WohlfuehlDorfIslandExperience() {
    ...
    <div className="wohlfuehl-dorf-island">
      <div className="island-header">
        ...
        <h1 className="island-title">
          <motion.div className="title-icon" animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 3 }}><WohlfuehlDorfIcon size={40} animated /></motion.div>
      ...
      <motion.div className="island-subtitle-bar" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <span className="subtitle-text">(Zugehörigkeit &amp; Wohlbefinden)</span>
      </motion.div>
