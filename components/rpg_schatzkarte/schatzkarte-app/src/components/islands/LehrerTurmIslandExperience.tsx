...
import { LehrerTurmIcon } from '@/components/icons';
...
const QUEST_INFO: Record<QuestKey, { name: string; icon: string; color: string; description: string; xp: number }> = {
  video: { name: 'Weisheit erlangen', icon: '📜', color: '#b39ddb', description: 'Schau dir das Lernvideo an!', xp: 25 },
  scroll: { name: 'Schriftrolle studieren', icon: '📖', color: '#9575cd', description: 'Lerne über Lehrer-Schüler-Beziehung!', xp: 20 },
  quiz: { name: 'Wissen testen', icon: '⚔️', color: '#7e57c2', description: 'Teste dein Wissen!', xp: 50 },
  challenge: { name: 'Kommunikations-Übung', icon: '🏫', color: '#673ab7', description: 'Übe bessere Kommunikation!', xp: 40 },
};

export function LehrerTurmIslandExperience() {
    ...
    <div className="lehrer-turm-island">
      <div className="island-header">
        ...
        <h1 className="island-title">
          <motion.div className="title-icon" animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 3 }}><LehrerTurmIcon size={40} animated /></motion.div>
      ...
      <motion.div className="island-subtitle-bar" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <span className="subtitle-text">(Lehrer-Schüler-Beziehung)</span>
      </motion.div>
