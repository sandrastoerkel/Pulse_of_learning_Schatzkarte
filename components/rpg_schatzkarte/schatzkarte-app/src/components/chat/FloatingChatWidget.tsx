...
export default function FloatingChatWidget({ groupId, groupName, allGroups }: FloatingChatWidgetProps) {
  // Auth-Daten aus globalem Context
  const { legacyUserId, profile } = useAuth();
  const userId = legacyUserId ?? '';
  const userName = profile?.display_name ?? 'Unbekannt';
  const userRole: 'coach' | 'kind' = profile?.role === 'coach' ? 'coach' : 'kind';
  ...
  // Initial: Nachrichten + Mitglieder fuer aktive Gruppe laden
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const loaded: ChatMessage[] = (msgData || [])
          .reverse()
          .filter((m: any) => {
            if (!m.recipient_id) return true;
            return m.recipient_id === userId || m.sender_id === userId;
        ...
        if (userRole === 'coach' && !mems.some(m => m.userId === userId)) {
          mems.push({ userId, name: userName, role: 'coach' });
        }
        setMembers(mems);
      } catch (err) {
        console.warn('Fehler beim Laden der Chat-Daten:', err);
      }
    })();
