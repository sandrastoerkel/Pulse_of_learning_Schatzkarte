// ============================================
// Weniger Stress beim Lernen Island Experience
...
import { RuheOaseIcon } from './icons';
import '../styles/ruhe-oase-island.css';

type QuestKey = 'video' | 'scroll' | 'quiz' | 'challenge';

interface RuheOaseProgress {
  completedQuests: QuestKey[];
  totalXP: number;
}

interface RuheOaseIslandProps {
  ageGroup: AgeGroup;
  onClose: () => void;
...
const QUEST_INFO: Record<QuestKey, { name: string; icon: string; color: string; description: string; xp: number; }> = {
  video: { name: "Weisheit erlangen", icon: "📜", color: "#4dd0e1", description: "Schau dir das Lernvideo an!", xp: 25 },
  scroll: { name: "Schriftrolle studieren", icon: "📖", color: "#26c6da", description: "Lerne über Entspannung!", xp: 20 },
  quiz: { name: "Wissen testen", icon: "⚔️", color: "#00bcd4", description: "Teste dein Wissen!", xp: 50 },
  challenge: { name: "Atem-Übung", icon: "🌬️", color: "#00acc1", description: "Finde deine innere Ruhe!", xp: 40 },
};

export function RuheOaseIslandExperience({ ageGroup, onClose, onQuestComplete }: RuheOaseIslandProps) {
  const [currentView, setCurrentView] = useState<'overview' | QuestKey>('overview');
  const [progress, setProgress] = useState<RuheOaseProgress>({ completedQuests: [], totalXP: 0 });
  ...
  return (
    <div className="ruhe-oase-island">
      <div className="island-header">
        <button className="back-btn" onClick={onClose}>← Zurück</button>
        <h1 className="island-title">
          <motion.div className="title-icon" animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 3 }}><RuheOaseIcon size={40} animated={true} /></motion.div>
      ...
      <motion.div className="island-subtitle-bar" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <span className="subtitle-text">(Entspannung & Stressabbau)</span>
      </motion.div>
...
export default RuheOaseIslandExperience;
